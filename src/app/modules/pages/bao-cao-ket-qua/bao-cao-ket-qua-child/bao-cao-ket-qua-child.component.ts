...
@Component({
  selector: 'app-bao-cao-ket-qua-child',
  templateUrl: './bao-cao-ket-qua-child.component.html',
  styleUrls: ['./bao-cao-ket-qua-child.component.css']
})
export class BaoCaoKetQuaChildComponent implements OnInit {
  ...
  navigateRouteDetails(duAn: string){
   
    this.router.navigate([`/pages/bao-cao-ket-qua/details/${duAn}`]);
  ...
  async callApiReport(rpCode: string): Promise<any> {
   ...
   async getDAByCode() {
    ...
    if(this.duAnChildList.length >0) {
     
      for(let i = 0; i < this.duAnChildList.length; i++) {
        console.log(this.duAnChildList[i])
        let data: TT01DataExport[] = await this.callApiReport(this.duAnChildList[i])
        this.dataChartArray.push(data)
  ...
  async ngOnInit() {
    ...
    await this.route.queryParams.subscribe(params => {
      if(typeof params['child'] == 'string') {
        this.duAnChildList.push(params['child'])
      }
      else {this.duAnChildList = params['child'];}
