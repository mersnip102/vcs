...
import { NhomNguoiDungService } from 'src/app/services/nhom-nguoi-dung/nhom-nguoi-dung.service';
...
export class NhomNguoiDungComponent implements OnInit {
  ...
  showModal(id: any): void {
    ...
    if(id == -1) {
      ...
      console.log(id)
      this.tenNhom = this.listNhomNguoiDung.find((item: any) => item.id == id).ten_nhom
      console.log(this.tenNhom)
      this.moTa = this.listNhomNguoiDung.find((item: any) => item.id == id).mo_ta
    }
    this.isVisible = true;
  ...
  handleOk(): void {
    ...
    if (this.selectedGroupId == -1) {
      ...
      this.api.createNhomNguoiDung(data)
        .subscribe(
          response => {
  ...
  getNhomNguoiDungList() {
    ...
    this.api.getNhomNguoDungList().subscribe((res: any) => {
      this.listNhomNguoiDung = res.data;

      console.log(this.listNhomNguoiDung);
    }, error => {
  ...
  deleteNhomNguoiDung(id: any) {
    this.notifyService.confirmDelete().then((result) => {
      if (result) {
        this.api.deleteNhomNguoiDung(id).subscribe((res: any) => {
  ...
  constructor(
    ...
    private api: NhomNguoiDungService,
